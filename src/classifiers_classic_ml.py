...
def visualize_embeddings(X_train, X_test, y_train, y_test, plot_type='2D', method='PCA'): 
    ...
    if plot_type == '3D':
        ...
        # Ensure y_test is 1D and matches the length of reduced_embeddings
        y_test_1d = np.array(y_test).flatten()
        # Make sure lengths match (take the correct slice if needed)
        if len(y_test_1d) != len(reduced_embeddings):
            y_test_1d = y_test_1d[:len(reduced_embeddings)]
        df_reduced['Class'] = y_test_1d
...
def train_and_evaluate_model(X_train, X_test, y_train, y_test, models=None, test=True):
    ...
    # Flatten labels if they are 2D (e.g., from DataFrame with single column)
    ...
    print(f"Flattened y_train_flat shape: {y_train_flat.shape}, y_test_flat shape: {y_test_flat.shape}")
    
    visualize_embeddings(X_train, X_test, y_train_flat, y_test_flat, plot_type='2D', method='PCA')
    ...
    for name, model in models:
        ...
        if test:
            accuracy, precision, recall, f1 = test_model(X_test, y_test_flat, model)
