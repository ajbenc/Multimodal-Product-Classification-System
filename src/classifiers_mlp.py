...
def train_mlp(train_loader, test_loader, text_input_size, image_input_size, output_size, num_epochs=50, report=False, lr=0.001, set_weights=True, adam=False, p=0.0, seed=1, patience=40, save_results=True, train_model=True, test_mlp_model=True):
    ...
    hidden = 256  # Set default hidden layer size
    ...
    if train_model:
        history = model.fit(train_loader, validation_data=test_loader, epochs=num_epochs, callbacks=[early_stopping], class_weight=class_weights if set_weights else None, verbose=1)
    ...
    if test_mlp_model:
        ...
        for batch in test_loader:
            ...
            if len(features) == 1:
                text = features['text_input'] if 'text_input' in features else features['image_input']
...
def train_mlp_with_finetuned_images(train_loader, test_loader, output_size, num_epochs=50, report=False, lr=0.001, p=0.0, seed=1, patience=40, save_results=True, train_model=True, test_mlp_model=True):
    """
    Trains a multimodal early fusion model using text data and fine-tuned image features.

    This function is similar to `train_mlp`, but specifically uses pre-trained and fine-tuned image features
    (e.g., from a ResNet50 model) for the image data. The text data is processed as usual. The model is trained
    to combine these features for the final classification task.

    Args:
        train_loader (MultimodalDataset): Keras-compatible data loader for the training set with text data and fine-tuned image features.
        test_loader (MultimodalDataset): Keras-compatible data loader for the test set with text data and fine-tuned image features.
        output_size (int): Number of output classes for the softmax layer.
        num_epochs (int, optional): Number of training epochs. Default is 50.
        report (bool, optional): Whether to generate a detailed classification report and display metrics. Default is False.
        lr (float, optional): Learning rate for the optimizer. Default is 0.001.
    ...
    Training Process:
        - The function creates a fusion model combining text and fine-tuned image inputs.
        - The model is trained using categorical cross-entropy loss and the Adam optimizer.
        - Early stopping is applied based on validation loss to prevent overfitting.
        - After training, the model is evaluated on the test set, and accuracy, F1-score, and AUC are calculated.

    Example:
        train_mlp_with_finetuned_images(train_loader, test_loader, output_size=10, num_epochs=30, lr=0.001, report=True)

    Notes:
        - `train_loader` and `test_loader` should be instances of `MultimodalDataset` or compatible Keras data loaders.
        - This function assumes that the image features have already been fine-tuned and are being used as-is for the task.
    """
    ...
    # TODO: Create an early fusion model using the provided input sizes and output size
    hidden = 256  # Set default hidden layer size
    text_input_size = train_loader.text_data.shape[1] if train_loader.text_data is not None else None
    image_input_size = train_loader.image_data.shape[1] if train_loader.image_data is not None else None
    model = create_early_fusion_model(text_input_size, image_input_size, output_size, hidden, p)

    # TODO: Load the fine-tuned model weights
    model.load_weights('best_finetuned_model.h5')

    # TODO: Compile the model with the Adam optimizer and categorical cross-entropy loss
    ...
    # Use the number of epochs specified
    if train_model:
        history = model.fit(train_loader, validation_data=test_loader, epochs=num_epochs, callbacks=[early_stopping], verbose=1)
...
def use_finetuned_model_for_image_predictions(test_df, model_path='best_finetuned_model.h5', report=False):
    """
    Uses the fine-tuned ResNet50 model to generate predictions and save results for test submission.
    
    This function loads the complete fine-tuned ResNet50 model (not just weights) and generates
    predictions directly from raw images. This is the correct way to integrate transfer learning
    results with the assignment requirements.

    Args:
        test_df (pd.DataFrame): Test dataframe with 'image_path' and 'category' columns
        model_path (str, optional): Path to the saved fine-tuned model. Default is 'best_finetuned_model.h5'.
        report (bool, optional): Whether to generate detailed classification report. Default is False.

    Returns:
        tuple: (model, test_accuracy, f1, macro_auc)
            - model: The loaded fine-tuned ResNet50 model
            - test_accuracy: Accuracy on test set
            - f1: Macro F1 score on test set
            - macro_auc: Macro AUC score on test set

    Example:
        model, acc, f1, auc = use_finetuned_model_for_image_predictions(test_df, report=True)
        print(f"Test Accuracy: {acc:.4f}, F1: {f1:.4f}, AUC: {auc:.4f}")

    Notes:
        - This function requires the fine-tuned model file to exist
        - Saves predictions to 'results/image_results.csv' for test submission
        - Uses ImageDataGenerator to load and preprocess images on-the-fly
    """
    from tensorflow.keras.models import load_model
    from tensorflow.keras.preprocessing.image import ImageDataGenerator
    
    print("Loading fine-tuned ResNet50 model...")
    # Load the COMPLETE fine-tuned model (not just weights)
    ...
    # Convert to one-hot for compatibility with test_model function
    ...
    if report:
        # Create a simple encoder for compatibility
        class SimpleEncoder:
        ...
        encoder = SimpleEncoder(list(test_generator.class_indices.keys()))
        test_model(y_true_onehot, y_pred, y_prob, encoder=encoder)
    
    # Save results for test submission
    results = pd.DataFrame({
        'Predictions': y_pred,
        'True Labels': y_true
    })
    os.makedirs('results', exist_ok=True)
    results.to_csv("results/image_results.csv", index=False)
    print("✅ Results saved to results/image_results.csv")
    
    return model, test_accuracy, f1, macro_auc

def use_finetuned_model_for_predictions(test_df, model_path='best_finetuned_model.h5'):
    """
    Load fine-tuned ResNet50 and generate predictions directly
    """
    from tensorflow.keras.models import load_model
    from tensorflow.keras.preprocessing.image import ImageDataGenerator
    ...
    results = pd.DataFrame({
        'Predictions': y_pred,
        'True Labels': test_generator.classes
    })
    os.makedirs('results', exist_ok=True)
    results.to_csv("results/image_results.csv", index=False)
